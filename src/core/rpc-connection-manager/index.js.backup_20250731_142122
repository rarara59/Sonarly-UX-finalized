...
import { NativeWebSocketClient } from '../../services/native-websocket-client.js';
import { HeliusWebSocketClient } from '../../services/production-helius-websocket-client.js'; // Updated file name

// Mock config for development
const config = {
  solanaRpcEndpoint: process.env.HELIUS_RPC_URL || 'https://rpc.helius.xyz/?api-key=YOUR_HELIUS_KEY'
};
...
class WebSocketManager extends EventEmitter {
  ...
  /**
   * Health check method for orchestrator compatibility
   */
  async healthCheck() {
    try {
      const stats = this.getWebSocketStats();
      const hasActiveConnections = stats.overview.activeConnections > 0;
      const hasHealthyConnections = Array.from(this.connectionStates.values())
        .some(state => state === 'connected');
      
      // WebSocket is healthy if we have active connections or WebSocket is disabled
      const isHealthy = hasActiveConnections || hasHealthyConnections || !this.rpcManager.options.enableWebSocket;
      
      return isHealthy;
    } catch (error) {
      this.rpcManager.logger.warn('WebSocketManager healthCheck failed:', error.message);
      return false;
    }
  }
  
  /**
   * Simple health status method
   */
  isHealthy() {
    try {
      const hasActiveConnections = this.connections.size > 0;
      const hasHealthyStates = Array.from(this.connectionStates.values())
        .some(state => state === 'connected');
      
      // Return true if we have connections or if WebSocket is disabled
      return hasActiveConnections || hasHealthyStates || !this.rpcManager.options.enableWebSocket;
    } catch (error) {
      return false;
    }
  }
...
class BasicMemoryMonitor extends EventEmitter {
  constructor(logger) {
    ...
    this.thresholds = {
      warning: 400 * 1024 * 1024,    // 400MB (reduced from 512MB)
      critical: 800 * 1024 * 1024,   // 800MB (reduced from 1GB)
      emergency: 1200 * 1024 * 1024  // 1.2GB (reduced from 1.5GB)
    };
    
    this.options = {
      checkInterval: 15000,        // 15 seconds (more frequent)
      alertCooldown: 180000,       // 3 minutes (reduced from 5)
      forceGCThreshold: 0.7,       // 70% of warning threshold (more aggressive)
...
const METHOD_CATEGORIES = {
  ...
  'getTokenAccountBalance': MethodCategory.TOKEN_INFO,
...
class RPCConnectionManager extends EventEmitter {
  ...
  /**
   * Get singleton instance of RPCConnectionManager
   * @param {Object} options - Configuration options (only used on first call)
   * @returns {RPCConnectionManager} Singleton instance
   */
  static getInstance(options = {}) {
    if (!RPCConnectionManager.instance) {
      console.log('🔧 Creating singleton RPCConnectionManager instance');
      RPCConnectionManager.instance = new RPCConnectionManager(options);
    } else if (Object.keys(options).length > 0) {
      console.warn('⚠️ RPCConnectionManager already initialized. Ignoring new options.');
    }
  ...
  static resetInstance() {
    if (RPCConnectionManager.instance) {
      console.log('🔄 Resetting RPCConnectionManager singleton instance');
      RPCConnectionManager.instance.shutdown().catch(console.error);
      RPCConnectionManager.instance = null;
    }
  }

  constructor(options = {}) {
    // DEBUG: RPCConnectionManager constructor started
    
    // Enforce singleton pattern
    if (RPCConnectionManager.instance) {
      console.warn('⚠️ RPCConnectionManager instance already exists! Use getInstance() instead.');
      return RPCConnectionManager.instance;
    }
    ...
    // DEBUG: Initializing circuit breakers...
    ...
    // DEBUG: Initializing Solana connections...
    ...
    // DEBUG: Setting up rate limits...
    this.initializeRateLimits();
    
    // DEBUG: Starting request processor...
    ...
    // DEBUG: Initializing WebSocket connections...
    // Initialize WebSocket connections and mark as ready
    this.initializationPromise = this.initializeWebSocketConnections();
    this.isReady = false;
    this.initializationPromise.then(() => {
      // DEBUG: WebSocket initialization completed
      this.isReady = true;
    }).catch(error => {
      // DEBUG: WebSocket initialization failed
      this.logger.error('Initialization failed:', error);
    });
    
    // DEBUG: RPCConnectionManager constructor completed
  ...
  async initialize() {
    ...
    console.log('RPCConnectionManager initialized and ready');
  ...
  async healthCheck() {
    try {
      const endpointsHealthy = this.endpoints && Object.keys(this.endpoints).length > 0;
      const hasActiveEndpoints = this.endpoints && Object.values(this.endpoints).some(
        endpoint => endpoint.health > 50
      );
      // Defensive WebSocket health check
      let websocketHealthy = true;
      if (this.webSocketManager) {
        try {
          // Check if isHealthy method exists
          if (typeof this.webSocketManager.isHealthy === 'function') {
            websocketHealthy = this.webSocketManager.isHealthy();
          } else if (typeof this.webSocketManager.healthCheck === 'function') {
            // Fallback to async healthCheck if available
            websocketHealthy = await this.webSocketManager.healthCheck();
          } else {
            // Fallback to simple connection status check
            // If WebSocket is disabled, consider it healthy (non-critical)
            websocketHealthy = this.webSocketManager.isConnected === true || 
                            this.webSocketManager.isInitialized === true ||
                            (!this.options.enableWebSocket) ||  // WebSocket disabled = healthy
                            true;  // Default to healthy for non-critical service
            this.logger.debug('WebSocket manager using fallback health check');
          }
        } catch (error) {
          this.logger.warn('WebSocket health check error:', error.message);
          // If there's an error, assume WebSocket is not critical for operation
          websocketHealthy = true;
        }
      }
      const circuitBreakersHealthy = this.circuitBreakers && Object.values(this.circuitBreakers).some(
        cb => cb.state !== 'OPEN'
      );
      
      const isHealthy = endpointsHealthy && hasActiveEndpoints && websocketHealthy && circuitBreakersHealthy;
      
      this.emit('healthCheck', {
        healthy: isHealthy,
        endpointsCount: this.endpoints ? Object.keys(this.endpoints).length : 0,
        activeEndpoints: hasActiveEndpoints,
        websocketHealthy: websocketHealthy,
        circuitBreakersHealthy: circuitBreakersHealthy,
        timestamp: Date.now()
      });
      ...
      console.error('RPCConnectionManager health check failed:', error);
      return false;
    }
  }
  ...
  initializeEndpoints() {
    return {
      ...
      chainstack: {
        url: process.env.HELIUS_RPC_URL || config.solanaRpcEndpoint,
        apiKey: process.env.HELIUS_API_KEY || '',
        rateLimit: 1000,
        active: Boolean(process.env.HELIUS_RPC_URL || config.solanaRpcEndpoint),
        health: 100,
        responseTime: 0,
        failCount: 0,
        lastCall: 0,
  ...
  validateSolanaParameters(method, params) {
    const validationRules = {
      ...
      'getTokenAccountBalance': (params) => {
        if (!params[0] || !this.validatePublicKey(params[0])) {
          throw new Error(`Invalid token account address for getTokenAccountBalance: ${params[0]}`);
        }
      }
    };
  ...
  async executeSolanaMethod(connection, method, params) {
    switch (method) {
      ...
      case 'getTokenAccountBalance':
        return await connection.getTokenAccountBalance(new PublicKey(params[0]));
  ...
  startMetricsExport() {
    ...
    const metricsServer = http.createServer((req, res) => {
      if (req.url === '/metrics') {
        ...
        res.end(JSON.stringify({
          ...
          webSocket: this.webSocketManager && typeof this.webSocketManager.getWebSocketStats === 'function' 
            ? this.webSocketManager.getWebSocketStats() 
            : { overview: { activeConnections: 0, totalSubscriptions: 0 }, traffic: { messagesReceived: 0 } }
        }));
      } else {
        res.writeHead(404);
        res.end('Not Found');
    ...
    const metricsPort = process.env.METRICS_PORT || (9090 + Math.floor(Math.random() * 100));
    ...
    setInterval(() => {
      ...
      const wsStats = this.webSocketManager && typeof this.webSocketManager.getWebSocketStats === 'function' 
        ? this.webSocketManager.getWebSocketStats() 
        : { overview: { activeConnections: 0, totalSubscriptions: 0 }, traffic: { messagesReceived: 0 } };
  ...
  exportPrometheusMetrics() {
    ...
    // WebSocket metrics (with defensive check)
    if (this.webSocketManager && typeof this.webSocketManager.getWebSocketStats === 'function') {
      try {
        const wsStats = this.webSocketManager.getWebSocketStats();
        metrics.push(`rpc_websocket_connections ${wsStats.overview?.activeConnections || 0}`);
        metrics.push(`rpc_websocket_subscriptions ${wsStats.overview?.totalSubscriptions || 0}`);
        metrics.push(`rpc_websocket_messages_received ${wsStats.traffic?.messagesReceived || 0}`);
      } catch (error) {
        this.logger.debug('WebSocket stats error:', error.message);
        metrics.push(`rpc_websocket_connections 0`);
        metrics.push(`rpc_websocket_subscriptions 0`);
        metrics.push(`rpc_websocket_messages_received 0`);
      }
    } else {
      metrics.push(`rpc_websocket_connections 0`);
      metrics.push(`rpc_websocket_subscriptions 0`);
  ...
  /**
   * Get metrics for external services (simplified interface)
   * @returns {Object} Metrics object with key statistics
   */
  getMetrics() {
    const endpointStatuses = this.getEndpointStatuses();
    const activeEndpoints = Object.values(endpointStatuses).filter(ep => ep.active).length;
    const memoryStats = this.memoryMonitor.getMemoryStats();
    const connectionStats = this.connectionTracker.getConnectionStats();
    
    // Calculate average latency across all active endpoints
    const activeEndpointsList = Object.entries(endpointStatuses)
      .filter(([_, status]) => status.active && status.responseTime > 0);
    const averageLatency = activeEndpointsList.length > 0
      ? activeEndpointsList.reduce((sum, [_, status]) => sum + status.responseTime, 0) / activeEndpointsList.length
      : 0;
    
    // Determine current primary endpoint
    const currentEndpoint = Object.entries(endpointStatuses)
      .filter(([_, status]) => status.active)
      .sort((a, b) => (b[1].health * b[1].priority) - (a[1].health * a[1].priority))[0]?.[0] || 'none';
    ...
    return {
      ...
      activeConnections: connectionStats.totals.active,
      totalConnections: connectionStats.totals.total,
      connectionErrors: connectionStats.totals.errors,
      
      // Memory usage
      memoryUsageMB: memoryStats.current.heapUsed,
      memoryPeakMB: memoryStats.stats.peakMemory,
      
      // System uptime
      uptimeSeconds: Math.round(process.uptime()),
      
      // Cache statistics
      cacheSize: this.fastCache.getStats().cacheSize,
      cacheHitRate: this.fastCache.getStats().totalAccesses > 0 
        ? (this.fastCache.getStats().cacheSize / this.fastCache.getStats().totalAccesses) 
        : 0,
      
      // Request statistics
      totalRequests: Object.values(endpointStatuses).reduce((sum, status) => sum + status.callCount, 0),
      failedRequests: Object.values(endpointStatuses).reduce((sum, status) => sum + status.failCount, 0),
      
      // Health status
      healthy: this.healthCheck ? true : false,
      status: activeEndpoints > 0 ? 'operational' : 'degraded'
    };
  }
  ...
  /**
   * Get program accounts with filtering and configuration options
   * Used for pool scanning and LP discovery functionality
   * 
   * @param {string} programId - The program ID to query accounts for
   * @param {Object} options - Configuration options including filters, dataSlice, etc.
   * @param {string} priority - Request priority ('high', 'normal', 'low')
   * @returns {Promise<Array>} Array of account objects with pubkey and account data
   */
  async getProgramAccounts(programId, options = {}, priority = 'normal') {
    if (!programId) {
      throw new Error('Program ID is required for getProgramAccounts');
    }

    if (!this.validatePublicKey(programId)) {
      throw new Error(`Invalid program ID for getProgramAccounts: ${programId}`);
    }

    try {
      // Use the existing call infrastructure for consistency with circuit breaker protection
      const result = await this.call('getProgramAccounts', [programId, {
        encoding: 'base64',
        commitment: 'confirmed',
        ...options
      }], undefined, undefined, priority);

      // Handle the response format - should be array of account objects
      if (!Array.isArray(result)) {
        this.logger.warn('getProgramAccounts returned non-array result', { programId, resultType: typeof result });
        return [];
      }

      // Log successful execution for monitoring
      this.logger.debug('getProgramAccounts executed successfully', {
      ...
      const errorMessage = `getProgramAccounts failed for program ${programId.slice(0, 8)}...: ${error.message}`;
      ...
      enhancedError.programId = programId;
  ...
  async shutdown() {
    ...
    try {
      ...
      // Shutdown WebSocket connections (defensive)
      if (this.webSocketManager && typeof this.webSocketManager.shutdown === 'function') {
        try {
          await this.webSocketManager.shutdown();
        } catch (error) {
          this.logger.warn('WebSocket shutdown error:', error.message);
        }
      }
...
// 🔒 Global symbol key so any module gets the same instance
const GLOBAL_KEY = Symbol.for('thorp.rpcManager');

// Lazy initialization to prevent hanging on import
let rpcSingleton = null;

const getRPCManager = () => {
  if (!rpcSingleton) {
    // DEBUG: Creating RPCConnectionManager singleton...
    rpcSingleton = new RPCConnectionManager();
    globalThis[GLOBAL_KEY] = rpcSingleton;
  }
  return rpcSingleton;
};
...
if (globalThis[GLOBAL_KEY]) {
  rpcSingleton = globalThis[GLOBAL_KEY];
}

// Export the manager directly for backward compatibility
// but it will be lazily created on first access
