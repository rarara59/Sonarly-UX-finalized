/**
 * MATH WORKER THREAD - PRODUCTION GRADE
 *
 * Dedicated worker for CPU-intensive mathematical operations only.
 * NO network calls, NO imports, NO external dependencies.
 * Pure mathematical processing for maximum performance and stability.
 *
 * Supported Operations:
 * - calculatePrice            (price calculations from reserve/sqrt data)
 * - calculateTVL              (liquidity / TVL math)  
 * - parsePoolData             (binary data parsing - PRE-FETCHED data only)
 * - mathematicalOperations    (Renaissance math engine)
 * - validatePoolInvariants    (security/validation checks)
 * - riskCalculations          (risk engine calculations)
 */

import { parentPort, workerData, isMainThread } from 'worker_threads';

// CRITICAL: Prevent execution outside worker context
if (isMainThread || !parentPort) {
  console.log('⚠️  Math worker should not be imported directly in main thread');
...
console.log(`Math worker ${workerId} started`);
...
/**
 * Parse pool data that has already been fetched from RPC
 * Input: Pre-processed data with raw buffer and metadata
 */
function parsePoolData(data) {
  const { poolType, accountDataBuffer, layoutConstants } = data;
  
  if (!accountDataBuffer) {
    throw new Error('Missing accountDataBuffer in parsePoolData');
  }
  ...
  switch (poolType) {
    case 'raydium':
      return parseRaydiumPoolBuffer(buffer, layoutConstants);
    case 'orca':
      return parseOrcaPoolBuffer(buffer, layoutConstants);
    default:
...
/**
 * Parse Raydium pool from raw buffer data
 */
function parseRaydiumPoolBuffer(buffer, constants) {
  if (buffer.length < 752) {
    throw new Error(`Invalid Raydium pool data length: ${buffer.length}`);
  }

  const status        = readUInt64LE(buffer, constants.STATUS_OFFSET);
  const baseDecimals  = readUInt64LE(buffer, constants.BASE_DECIMALS_OFFSET);
  const quoteDecimals = readUInt64LE(buffer, constants.QUOTE_DECIMALS_OFFSET);

  // Extract public key bytes (main thread will convert to actual PublicKeys)
  const baseMintBytes  = buffer.slice(constants.BASE_MINT_OFFSET,  constants.BASE_MINT_OFFSET  + 32);
  const quoteMintBytes = buffer.slice(constants.QUOTE_MINT_OFFSET, constants.QUOTE_MINT_OFFSET + 32);
  const lpMintBytes    = buffer.slice(constants.LP_MINT_OFFSET,    constants.LP_MINT_OFFSET    + 32);
  const baseVaultBytes = buffer.slice(constants.BASE_VAULT_OFFSET,  constants.BASE_VAULT_OFFSET  + 32);
  const quoteVaultBytes = buffer.slice(constants.QUOTE_VAULT_OFFSET, constants.QUOTE_VAULT_OFFSET + 32);

  return {
    isLiquidityPool: Number(status) === 6,
    dex: 'raydium',
    baseMintBytes: Array.from(baseMintBytes),
    quoteMintBytes: Array.from(quoteMintBytes), 
    lpMintBytes: Array.from(lpMintBytes),
    baseVaultBytes: Array.from(baseVaultBytes),
    quoteVaultBytes: Array.from(quoteVaultBytes),
...
/**
 * Parse Orca whirlpool from raw buffer data
 */
function parseOrcaPoolBuffer(buffer, constants) {
  if (buffer.length < 653) {
    throw new Error(`Invalid Orca pool data length: ${buffer.length}`);
  }

  const tickSpacing = buffer.readUInt16LE(constants.TICK_SPACING_OFFSET);
  const feeRate     = buffer.readUInt32LE(constants.FEE_RATE_OFFSET);

  const liquidity      = readUInt128LE(buffer.slice(constants.LIQUIDITY_OFFSET,   constants.LIQUIDITY_OFFSET   + 16));
  const sqrtPriceX64   = readUInt128LE(buffer.slice(constants.SQRT_PRICE_OFFSET,  constants.SQRT_PRICE_OFFSET  + 16));
  const tickCurrentIdx = buffer.readInt32LE(constants.TICK_CURRENT_INDEX_OFFSET);

  // Extract public key bytes
  const tokenMintABytes = buffer.slice(constants.TOKEN_MINT_A_OFFSET, constants.TOKEN_MINT_A_OFFSET + 32);
  const tokenMintBBytes = buffer.slice(constants.TOKEN_MINT_B_OFFSET, constants.TOKEN_MINT_B_OFFSET + 32);
  const tokenVaultABytes = buffer.slice(constants.TOKEN_VAULT_A_OFFSET, constants.TOKEN_VAULT_A_OFFSET + 32);
  const tokenVaultBBytes = buffer.slice(constants.TOKEN_VAULT_B_OFFSET, constants.TOKEN_VAULT_B_OFFSET + 32);

  return {
    isLiquidityPool: liquidity > BigInt(0),
    dex: 'orca',
    poolType: 'whirlpool',
    tokenMintABytes: Array.from(tokenMintABytes),
    tokenMintBBytes: Array.from(tokenMintBBytes),
    tokenVaultABytes: Array.from(tokenVaultABytes),
    tokenVaultBBytes: Array.from(tokenVaultBBytes),
...
// Send ready signal
if (parentPort) {
  console.log(`Math worker ${workerId} ready for tasks`);
  parentPort.postMessage({
    type: 'worker_ready',
    workerId,
    timestamp: Date.now()
