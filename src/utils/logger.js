...
function getLogger() {
  if (!logger) {
    logger = pino({
      ...
      // Simplified configuration to prevent hanging
      // Remove timestamp and formatters that can cause issues
      redact: ['url', 'endpoint.url', 'apiKey', 'api_key']
    });
  }
  return logger;
}
...
export function logRpcOperation(requestId, endpoint, operation, startTime, outcome, error = null) {
  ...
  switch (outcome) {
    case 'ok':
      log.info('RPC operation completed', redactedLogData);
      break;
    case 'timeout':
    case 'rate_limited':
      log.warn('RPC operation degraded', redactedLogData);
      break;
    case 'breaker_open':
    case 'error':
      log.error('RPC operation failed', redactedLogData);
      break;
    default:
      log.warn('RPC operation unknown outcome', redactedLogData);
  }
}
...
export function logConnectionStats(endpoint, stats) {
  getLogger().debug('Connection statistics', {
    endpoint: endpoint,
...
export function logHealthProbe(endpoint, startTime, outcome, error = null) {
  ...
  if (outcome === 'ok') {
    getLogger().debug('Health probe completed', logData);
  } else {
    getLogger().warn('Health probe failed', logData);
  }
}
...
export function logSecretSafe(level, message, data = {}) {
  ...
  switch (level) {
    case 'debug':
      getLogger().debug(message, redactedData);
      break;
    case 'info':
      getLogger().info(message, redactedData);
      break;
    case 'warn':
      getLogger().warn(message, redactedData);
      break;
    case 'error':
      getLogger().error(message, redactedData);
      break;
    default:
      getLogger().info(message, redactedData);
  }
...
// Export proxy that lazily initializes logger
