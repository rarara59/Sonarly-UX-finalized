 ...
 */
  ...
  async analyzeRaydiumInstruction(discriminatorHex, instructionData, accounts, accountKeys, instructionIndex) {
    ...
    const lpIndicators = this.analyzeLPCreationIndicators(instructionData, accounts, accountKeys);
   ...
   */
  analyzeLPCreationIndicators(instructionData, accounts, accountKeys) {
  ...
  async parsePumpFunInstruction(instructionData, accounts, accountKeys, type) {
    try {
      ...
      if (type === 'create') {
        ...
        // For Pump.fun, accounts are already the actual addresses, not indices
  ...
  async applyRenaissanceMathematicalValidation(candidate, transaction) {
    ...
    try {
      ...
      console.log(`  - Bayesian probability: ${bayesianProbability.toFixed(3)} (threshold: ${this.options.bayesianConfidenceThreshold})`);
      console.log(`  - Significance score: ${significanceScore.toFixed(3)} (threshold: 0.70)`);
      console.log(`  - Entropy score: ${candidate.entropyScore.toFixed(3)} (threshold: ${this.options.entropyThreshold})`);
      console.log(`  - Overall confidence: ${overallConfidence.toFixed(3)} (threshold: ${this.options.accuracyThreshold})`);
      
      // Add validation checks with specific failure reasons
      if (bayesianProbability < this.options.bayesianConfidenceThreshold) {
          console.log(`❌ FAILED: Bayesian probability too low (${bayesianProbability.toFixed(3)} < ${this.options.bayesianConfidenceThreshold})`);
      }
      if (significanceScore < 0.70) {
          console.log(`❌ FAILED: Significance score too low (${significanceScore.toFixed(3)} < 0.70)`);
      }
      if (candidate.entropyScore < this.options.entropyThreshold) {
          console.log(`❌ FAILED: Entropy score too low (${candidate.entropyScore.toFixed(3)} < ${this.options.entropyThreshold})`);
      }
      if (overallConfidence < this.options.accuracyThreshold) {
          console.log(`❌ FAILED: Overall confidence too low (${overallConfidence.toFixed(3)} < ${this.options.accuracyThreshold})`);
      }
  ...
  testLPDetection() {
    ...
    const result = this.analyzeLPCreationIndicators(testInstructionData, testAccounts, testAccountKeys);
