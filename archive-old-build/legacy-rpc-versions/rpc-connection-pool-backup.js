...
class RpcConnectionPool extends EventEmitter {
  constructor(config = {}) {
    ...
    this.config = {
      // RPC Endpoints - Use dedicated nodes only, no public endpoints
      endpoints: (config.endpoints || [
        process.env.HELIUS_RPC_URL,
        // Chainstack temporarily disabled - authentication issue
    ...
    // Endpoint priorities (lower = higher priority)
    this.endpointPriorities = new Map();
    this.config.endpoints.forEach((endpoint, index) => {
      if (endpoint && (endpoint.includes('chainstack') || endpoint.includes('p2pify'))) {
        this.endpointPriorities.set(index, 1); // Highest priority - best latency
      } else if (endpoint && endpoint.includes('helius')) {
        this.endpointPriorities.set(index, 2); // Medium priority
      } else {
        this.endpointPriorities.set(index, 3); // Lowest priority
      }
    });
    ...
    // HTTP agents for connection pooling with optimized settings
    this.agents = new Map();
    if (this.config.keepAliveEnabled) {
      this.config.endpoints.forEach((endpoint, index) => {
        if (!endpoint) return; // Skip empty endpoints
        ...
        this.agents.set(index, new AgentClass({
          ...
          scheduling: 'fifo' // First-in-first-out for better latency
        }));
      });
    }
  ...
  async warmupConnections() {
    // Skip warmup if destroyed
    if (this.isDestroyed) return;
    
    // Warm up primary endpoint only to avoid delays
    try {
  ...
  /**
   * Select next available endpoint with circuit breaker check and priority
   */
  selectEndpoint() {
    ...
    for (let i = 0; i < this.config.endpoints.length; i++) {
      const breaker = this.circuitBreakers.get(i);
      if (!breaker) continue;
      ...
      if (isAvailable) {
        availableEndpoints.push({
          index: i,
          priority: this.endpointPriorities.get(i) || 999
        });
      }
    }
    ...
    if (availableEndpoints.length > 0) {
      return availableEndpoints[0].index;
    }
